package org.eclipse.imp.leg.occurrenceMarker;
...
public class LEGOccurrenceMarker implements ILanguageService, IOccurrenceMarker {
    private List fOccurrences= Collections.EMPTY_LIST;
    private IAst decl;

    public String getKindName() {
        return "LEG Occurence Marker";
    }
    ...
    public boolean nodeKindIndicatesAUniqueOccurrence(Object ast) {
        ...
        if (ast instanceof block || ast instanceof functionHeader
    ...
    public boolean nodeKindIndicatesComputedOccurrences(Object ast) {
        ...
        // same declaration.
        if (ast instanceof identifier || ast instanceof functionCall) {
            return true;
        }
        return false;
    }
    ...
    private class ComputedOccurrenceVisitor extends AbstractVisitor {
        ...
        public boolean visit(functionDeclarationList n) {
            if (ast instanceof functionCall) {
                ...
                functionName= ((functionCall) ast).getidentifier()
                ...
                if (identifierDeclTarget != null)
                    identifierDeclOffset= identifierDeclTarget.getLeftIToken()
                            .getStartOffset();
            }
            return true;
        }
        ...
        public void endVisit(functionDeclarationList n) {
            if (lookingForFunctionDecls) {
                if (functionDeclTarget == null) {
                    ...
                    List<functionCall> functionCalls= callsWithDeclarations
                            .get(functionDeclTarget);
            ...
            if (ast instanceof functionCall) {
                ...
                if (n.equals(ast)) {
                    if (functionDecls.empty()) {
                        ...
                        if (!callsWithDeclarations
                                .containsKey(functionDeclTarget)) {
                            callList= new ArrayList<functionCall>();
                            callList.add(n);
                            callsWithDeclarations.put(functionDeclTarget,
                                    callList);
                        } else {
                            callList= callsWithDeclarations
                                    .get(functionDeclTarget);
                            callList.add(n);
                        }
                    }
                } else if (n.getidentifier().getIDENTIFIER().toString().equals(
                        functionName)) {
                    if (functionDecls.empty()) {
                        callsWithoutDeclarations.add(n);
                    } else {
                        List<functionCall> callList;
                        functionDeclaration currentDeclaration= functionDecls
                                .peek();
                        if (!callsWithDeclarations
                                .containsKey(currentDeclaration)) {
                            callList= new ArrayList<functionCall>();
                            callList.add(n);
                            callsWithDeclarations.put(currentDeclaration,
                                    callList);
                        } else {
                            callList= callsWithDeclarations
        ...
        public boolean visit(functionDeclaration n) {
            if (lookingForFunctionDecls
                    && n.getfunctionHeader().getidentifier().toString().equals(
                            functionName)) {
        ...
        public void endVisit(functionDeclaration n) {
            if (lookingForFunctionDecls && !functionDecls.empty()
                    && functionDecls.peek().equals(n))
                functionDecls.pop();
        }
